 ...
 * Modified by Cristhian.Larico.Info (c) 2012
...
(function($) {
  ...
  init: function(el, options) {
    var defaults = {
      ...
      auto            : true,
      autopause       : true,
      autopausetimer  : 5000, // add to delay for total restart time.
	  autoimagepause  : true,
      controlautopause: true,
      delay           : 3000, // in ms
	...
	if (this.options.controlautopause) {
		 $(".sexyslider-control").bind('mouseover', $.proxy(function() { this.setcontrolpause(true) }, this));
		 $(".sexyslider-control").bind('mouseout',  $.proxy(function() { this.setcontrolpause(false) }, this));
	}
	...
	if (this.options.autoimagepause) {
  ...
  transition: function(dir) {
    ...
    if (dir !== undefined)
    {
      if (this.options.autopause)
      {
		this.stopshow = true;
        clearInterval(this.imgInterval);
	    this.imgInterval = setInterval($.proxy(function() {clearInterval(this.imgInterval); this.stopshow = false; this.slideshow();}, this ), this.options.autopausetimer+((this.options.stripSpeed / 6)*this.options.strips)+this.options.stripSpeed);
